...
class DecoderTest < Minitest::Test
  ...
  def test_string
    strs = {
      ...
      "\x5e\x00\xd7".b + ('x'.b * 500) => 'x' * 500,
      "\x5e\x06\xb3".b + ('x'.b * 2000) => 'x' * 2000,
      "\x5f\x00\x10\x53".b + ('x'.b * 70_000) => 'x' * 70_000,
    }
    values = validate_type_decoding('string', strs)
    values.each do |s|
  ...
  def check_decoding(type, input, expected, name = nil)
    ...
    if %w[float double].include?(type)
      assert_in_delta(expected, r, 0.001, name)
    else
      assert_equal(expected, r, name)
    end
